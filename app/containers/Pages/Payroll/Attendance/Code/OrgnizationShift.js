...
function OrgnizationShift(props) {
  const { intl } = props;
  ...
  const handleClose =  async (Employeesdata) => {
   ...
   try{
    ...
    let response = await  ApiData(locale).SaveAll(shifts);
...
useEffect(() => {    
    ...
    async function getShiftData(id) 
    {
      ...
      const result = await shiftApi(locale).Get(id);
          ...
          setdata((prevFilters) => ({
              ...
              workHours: Math.round((new Date(0,0,0,result.endTime.split(':')[0],result.endTime.split(':')[1]) - new Date(0,0,0,result.startTime.split(':')[0],result.startTime.split(':')[1])) / 3600000)
  ...
  const options = {
    ...
    customToolbar: () => (
             <Button variant="contained"  size="medium" color="secondary" disabled={!data.shiftId} onClick={handleClickOpen} >               
    ...
    customToolbarSelect: (selectedRows) => (      
      <div>     
          ...
          <Button variant="contained" size="medium" color="primary" onClick={() => handleUpdate(selectedRows)} >
                    ...
                    direction="row">
                        <Grid item xs={12} md={4}>                    
                            <Autocomplete  
                                ...
                                onChange={(event, value) =>{ setdata((prevFilters) => ({
                                    ...prevFilters,
                        ...
                        <Grid item xs={12} md={2}>                    
                            ...
                            onChange={(e) => { setdata((prevFilters) => ({...prevFilters,workHours: e.target.value,}))}}                                                
...
OrgnizationShift.propTypes = {
  intl: PropTypes.object.isRequired,
};
export default injectIntl(OrgnizationShift);
