         ...
         if(event.target.name =="coreEnd")
        {
         ...
         if(event.target.name =="startTime")
         {
             ...
             if(data.endTime!="")
             {
                 ...
                 var diff = Math.round((new Date(0,0,0,data.endTime.split(':')[0],data.endTime.split(':')[1]) - new Date(0,0,0,event.target.value.split(':')[0],event.target.value.split(':')[1])) / 3600000);
         ...
         if(event.target.name =="endTime")
         {
             ...
             if(data.startTime!="")
             {
                 ...
                 var diff = Math.round((new Date(0,0,0,event.target.value.split(':')[0],event.target.value.split(':')[1]) - new Date(0,0,0,data.startTime.split(':')[0],data.startTime.split(':')[1])) / 3600000);
...
async function oncancel(){
  ...
  return (
    <div>
        <PapperBlock whiteBg icon="border_color" title={data.id==0?intl.formatMessage(messages.ShiftCreateTitle):intl.formatMessage(messages.ShiftUpdateTitle)} desc={""}>
