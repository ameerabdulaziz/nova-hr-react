...
const PermissionTrxData = (locale) => {
  ...
  Apis.getPermissions = async (data) => {
    const result = await axiosInstance.get(
      `AttPermissionTrx/getPermissions/${locale}?permissionId=${data.permissionId}&date=${data.date}&StartTime=${data.startTime}&EndTime=${data.endTime}`
    );
  ...
  Apis.SaveListFromImport = async (data) => {
    const result = await axiosInstance.post("AttPermissionTrx/SaveListFromImport", data);
    return result;
  };
  ...
  Apis.getRepeatedNo = async (permissionId, date, employeeId) => {
    const data = await axiosInstance.get(
      `AttPermissionTrx/getRepeatedNo?permissionId=${permissionId}&date=${date}&EmployeeId=${employeeId}`
    );
