...
function ChooseQuestionPopup(props) {
  const {
    intl, isOpen, setIsOpen, questionList, setFormInfo
  } = props;
  ...
  const getSelectedQuestions = useCallback(() => {
    ...
    return questionsClone.filter((item) => selectedQuestionsIds.includes(item.questionId)
    );
  }, [questionList, questionsClone]);

  const init = () => {
    const mappedQuestions = questionList.map((item) => ({
      ...item,
      isSelect: false,
      isNew: false,
    }));
  ...
  const searchAlgorithm = (question) => {
    ...
    const isExist = enName.includes(initQuery) || arName.includes(initQuery);
  ...
  const onFormSubmit = (evt) => {
    ...
    const selectedQuestions = getSelectedQuestions().map((item) => ({
      ...item,
      questionId: uuid(),
      choiceList: item.choiceList.map((item) => ({
        ...item,
        isNew: true,
      })),
    }));
  ...
  return (
    ...
    >
      <DialogTitle>
        {intl.formatMessage(surveyMessages.addQuestion)}
      </DialogTitle>
        ...
        <Grid container mb={3} gap={3} pt={2}>
          <Grid item xs={12} md={6}>
            <TextField
              ...
              label={intl.formatMessage(surveyMessages.question)}
              fullWidth
              variant='outlined'
              autoComplete='off'
            />
              ...
              <Table size='small' sx={{ minWidth: 700 }}>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>
                      {intl.formatMessage(surveyMessages.questionType)}
                    </TableCell>
                  </TableRow>
                </TableHead>
