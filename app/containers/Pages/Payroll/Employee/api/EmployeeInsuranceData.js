...
const EmployeeInsuranceData = (probs) => {
  ...
  EmployeeInsuranceApis.Save = async (Item) => {
    const data = {
      id: Item.id,
    ...
    const result =
      Item.id === 0
        ? await axiosInstance.post('EmpInsurance', data)
        : await axiosInstance.put(`EmpInsurance/${Item.id}`, data);
    return result;
  };
  ...
  return EmployeeInsuranceApis;
};

export default EmployeeInsuranceData;
