...
function RewardsPenaltyWidget(props) {
  ...
  const getdata = async () => {
    try {
      if (IsStaticDashboard == "false") {
        ...
        const data2 = await api(locale).getRewardPenalty();
        setData(data2);
      }
    } catch (error) {
    } finally {
      ...
      <PapperBlock whiteBg noMargin title={""} icon="timeline" desc="">
        <Grid item md={12} xs={12}>
          <Typography className={classes.smallTitle} variant="button">
            ...
            <FormattedMessage {...messages.RewardsPenaltyWidget} />
          </Typography>
...
export default RewardsPenaltyWidget;
