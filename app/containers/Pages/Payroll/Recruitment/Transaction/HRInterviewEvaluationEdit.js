...
function HRInterviewEvaluationEdit(props) {
  ...
  const [hrTests, setHrTests] = useState([]);
  ...
  async function fetchNeededData() {
    ...
    try {
      ...
      if (id) {
        ...
        setHrTests(response.jobApplicationHrtest);
  ...
  const onFormSubmit = async (evt) => {
    ...
    const hrTestsMapped = hrTests.map((item) => ({
      id: item.id,
  ...
  const onTestInputChange = (evt, index) => {
    const clonedTests = [...hrTests];

    const row = clonedTests[index];

    row[evt.target.name] = evt.target.value;

    clonedTests[index] = row;

    setHrTests(clonedTests);
  };

  const onNumericTestInputChange = (evt, index) => {
    const clonedTests = [...hrTests];
  ...
  const hrTestsFinalGrade = useMemo(() => {
    const reduce = hrTests.reduce(
      (acc, curr) => ({
        totalGrade: acc.totalGrade + parseFloat(curr.grade || 0),
      ...
      <form onSubmit={onFormSubmit}>
        ...
        {hrTests.length > 0 && (
          <PapperBlock
            ...
            icon='border_color'
            desc=''
            title={intl.formatMessage(messages.hrTests)}
          >
            <TableContainer>
              <Table sx={{ minWidth: 650 }}>
                ...
                <TableBody>
                  {hrTests.map((test, index) => {
                    const row = hrTests[index];
                      ...
                      >
                        ...
                        <TableCell sx={{ maxWidth: 120 }}>
                          <TextField
                            ...
                            className={classes.field}
                            onChange={(evt) => onNumericTestInputChange(evt, index)
                            }
                            variant='outlined'
                            required
                            autoComplete='off'
            ...
            <Typography variant='body2'>
              {intl.formatMessage(messages.finalGrade)}: {hrTestsFinalGrade} %
            </Typography>
          </PapperBlock>
        )}
...
HRInterviewEvaluationEdit.propTypes = {
  intl: PropTypes.object.isRequired,
};

export default injectIntl(HRInterviewEvaluationEdit);
