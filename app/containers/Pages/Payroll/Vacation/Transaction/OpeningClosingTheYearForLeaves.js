...
function OpeningClosingTheYearForLeaves(props) {
  ...
  const getdata = async () => {
    ...
    try {
      const Organizationlist = await GeneralListApis(locale).GetBranchList(
        true
      );
  ...
  const getDataById = async () => {
    ...
    try {
      const data = await OpeningClosingTheYearForLeavesData().GetDataById(
        Organization.id,
        Year.id
      );
  ...
  const openYearFun = async () => {
    ...
    try {
      let response = await OpeningClosingTheYearForLeavesData().SaveOpenYear(
        data
      );
  ...
  const closeYearFun = async () => {
    ...
    try {
      let response = await OpeningClosingTheYearForLeavesData().SaveCloseYear(
        data
      );
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={2}>
              <Button
                ...
                color="primary"
                disabled={
                  (!submittingOpenYear || !processingOpenYear) &&
                  (fromDate < toDate ||
            ...
            <Grid item xs={12} md={2}>
              <Button
                ...
                color="primary"
                disabled={
                  (!submittingCloseYear || !processingCloseYear) &&
                  (fromDate < toDate ||
                    (fromDate === null && toDate === null)) &&
                  apiData &&
                  apiData.id > 0 &&
                  apiData.closeDate === null
                    ? false
                    : true
                }
